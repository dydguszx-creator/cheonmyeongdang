export default async function handler(req, res) {
  ...
  // 디버그용 로그
  console.log('[천명당] 수신 데이터:', JSON.stringify({
    paymentId, email, name, year, month, day
  }));

  if (!paymentId) return res.status(400).json({ error: 'paymentId 누락' });
  if (!email) return res.status(400).json({ error: 'email 누락' });
...
【2. 타고난 성격·기질】본인만의 기질을 구체적 생활 장면으로.
...
.wrap{max-width:680px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)}
.cover{background:linear-gradient(160deg,#0d1a26,#050c12);padding:48px 36px;text-align:center}
.badge{font-size:11px;letter-spacing:4px;color:#C9A84C;margin-bottom:16px}
.title{font-size:26px;font-weight:900;color:#fff;line-height:1.5;margin-bottom:24px}
.title em{color:#C9A84C;font-style:normal}
.info{background:rgba(201,168,76,.08);border:1px solid rgba(201,168,76,.2);border-radius:12px;padding:18px 24px;display:inline-block;text-align:left;min-width:240px}
.row{display:flex;gap:14px;margin-bottom:7px;font-size:13px}
.row:last-child{margin-bottom:0}
.key{color:#C9A84C;width:56px;flex-shrink:0}
.val{color:#c0d0da}
.date{font-size:10px;color:#2a4050;letter-spacing:2px;margin-top:18px}
.body{padding:36px}
.text{font-size:14px;line-height:2.1;color:#333;word-break:keep-all;white-space:pre-wrap}
.footer{background:#0a1420;padding:28px 36px;text-align:center}
.brand{font-size:15px;font-weight:700;color:#C9A84C;letter-spacing:2px;margin-bottom:6px}
.note{font-size:11px;color:#3d5a6c;line-height:2}
</style>
</head>
<body>
<div class="wrap">
  <div class="cover">
    <div class="badge">✦ CHEONMYEONGDANG · 天命堂</div>
    <div class="title">${name} 님의<br/><em>사주 명리 분석 보고서</em></div>
    <div class="info">
      <div class="row"><span class="key">성 명</span><span class="val">${name}</span></div>
      <div class="row"><span class="key">성 별</span><span class="val">${gender}</span></div>
      <div class="row"><span class="key">생년월일</span><span class="val">${cal} ${year}년 ${month}월 ${day}일</span></div>
      <div class="row"><span class="key">출생시각</span><span class="val">${timeStr}</span></div>
    </div>
    <div class="date">발행일 · ${today}</div>
  </div>
  <div class="body">
    <div class="text">${analysisText.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br/>')}</div>
  </div>
  <div class="footer">
    <div class="brand">天命堂 천명당</div>
    <div class="note">천명당은 당신의 방향을 응원합니다 🌙<br/>본 보고서는 사주명리학 전통 이론에 기반한 참고 자료입니다.</div>
  </div>
</div>
</body>
</html>`;
    ...
    if (!resendRes.ok) {
      ...
      console.log('[천명당] 이메일 발송 오류:', err);
      return res.status(502).json({ error: '이메일 발송 실패', detail: err });
    }

    console.log(`[천명당] 완료: ${name} → ${email}`);
    return res.status(200).json({ success: true, message: `${email}로 보고서를 발송했습니다.` });

  } catch (e) {
    console.error('[천명당] 서버 오류:', e.message);
